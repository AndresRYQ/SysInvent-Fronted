...
function copiarTiposProducto(
  ...
  return tiposProducto.map((tipoProducto) => ({
    ...tipoProducto,
  }))
}
...
function crearSiguienteId(
  ...
  const numeroMayor = tiposProducto.reduce(
    (mayor, tipoProducto) => {
      const numero = Number(
        tipoProducto.id.replace('TP-', ''),
      )
  ...
  return `TP-${String(numeroMayor + 1).padStart(
    3,
    '0',
  )}`
}
...
export function obtenerTiposProducto(): TipoProducto[] {
  const datosGuardados =
    localStorage.getItem(STORAGE_KEY)

  if (!datosGuardados) {
    guardarTiposProducto(TIPOS_PRODUCTO_INICIALES)
...
export function crearTipoProducto(
  ...
  const nombreDuplicado = tiposProducto.some(
    (tipoProducto) =>
      normalizarTexto(tipoProducto.nombre) ===
      nombreNormalizado,
  )
  ...
  const nuevoTipoProducto: TipoProducto = {
    id: crearSiguienteId(tiposProducto),
...
export function actualizarTipoProducto(
  ...
  const tiposProducto = obtenerTiposProducto()
  const tipoProductoActual = tiposProducto.find(
  ...
  const tipoProductoActualizado: TipoProducto = {
    ...tipoProductoActual,
    nombre: datos.nombre.trim(),
    descripcion: datos.descripcion.trim(),
    estado: datos.estado,
...
export function eliminarTipoProducto(
  ...
  const existe = tiposProducto.some(
    (tipoProducto) => tipoProducto.id === id,
  )
