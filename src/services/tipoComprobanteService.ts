...
const TIPOS_DOCUMENTO_INICIALES: TipoComprobante[] = [
  {
    id: 'TD-001',
    nombre: 'Factura',
    descripcion:
...
function crearSiguienteId(
  ...
  const numeroMayor = tiposDocumento.reduce(
    (mayor, tipoDocumento) => {
      const numero = Number(
        tipoDocumento.id.replace('TD-', ''),
      )
  ...
  return `TD-${String(numeroMayor + 1).padStart(
    3,
    '0',
  )}`
}
...
export function crearTipoDocumento(
  ...
  const nuevoTipoDocumento: TipoComprobante = {
    id: crearSiguienteId(tiposDocumento),
