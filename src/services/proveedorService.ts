...
function crearSiguienteId(
  ...
  const numeroMayor = proveedores.reduce(
    (mayor, proveedor) => {
      const numero = Number(
        proveedor.id.replace('PROV-', ''),
      )
...
export function obtenerProveedores(): Proveedor[] {
  const datosGuardados =
    localStorage.getItem(STORAGE_KEY)

  if (!datosGuardados) {
    guardarProveedores(PROVEEDORES_INICIALES)
...
export function obtenerProveedorPorId(
  ...
  const proveedor = obtenerProveedores().find(
    (item) => item.id === id,
  )
...
export function crearProveedor(
  ...
  const rucDuplicado = proveedores.some(
    (proveedor) =>
      proveedor.ruc.trim() === datos.ruc.trim(),
  )
  ...
  const razonSocialDuplicada = proveedores.some(
    (proveedor) =>
      normalizarTexto(proveedor.razonSocial) ===
      normalizarTexto(datos.razonSocial),
  )
  ...
  const nuevoProveedor: Proveedor = {
    id: crearSiguienteId(proveedores),
    ruc: datos.ruc.trim(),
