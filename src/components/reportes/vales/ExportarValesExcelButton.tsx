...
interface ExportarValesExcelButtonProps {
...
function crearNombreArchivo(): string {
  ...
  return `reporte-vales-${fecha}.xlsx`
}
...
export function ExportarValesExcelButton({
  filas,
}: ExportarValesExcelButtonProps) {
  ...
  async function exportar(): Promise<void> {
    ...
    try {
      ...
      libro.title =
        'Reporte de vales de consumo'
      libro.subject =
        'Salidas y distribuciones de productos'
      libro.company =
        'Sistema de Inventario'
      ...
      hoja.headerFooter.oddHeader =
        '&C&BReporte de vales de consumo'
      ...
      hoja.getCell('A2').value =
        'Detalle de productos entregados, destinos y partes de equipo'
      ...
      filas.forEach(
        (fila, indice) => {
          ...
          filaExcel.getCell(16).numFmt =
            '"S/." #,##0.00'

          filaExcel.getCell(16).alignment = {
            vertical: 'middle',
            horizontal: 'right',
          }
